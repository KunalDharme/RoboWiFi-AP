...
Usage:
  sudo $0 SSID CHANNEL UPLINK_IF [MON_IF]
    SSID      - Fake AP name
...
ensure_monitor_mode() {
  ...
  # Try airmon-ng start (creates e.g. wlan0mon)
  if command -v airmon-ng >/dev/null 2>&1; then
    log_info "Attempting to enable monitor mode with airmon-ng..."
  ...
  # Fallback using iw
  log_info "Attempting to enable monitor mode with iw..."
...
# Wait for at0 to appear
log_info "Waiting for at0 interface..."
for i in {1..30}; do
  if ip link show at0 >/dev/null 2>&1; then
    log_info "at0 interface created successfully"
    break
  fi
  sleep 1
  [[ $((i % 5)) -eq 0 ]] && echo -n "."
...
if ! ip link show at0 >/dev/null 2>&1; then
  log_error "at0 interface not created"
...
# Configure at0
log_info "Configuring at0 with IP $AT0_IP/24"
ip addr flush dev at0 2>/dev/null || true
ip addr add "$AT0_IP/24" dev at0 2>/dev/null || true
ip link set at0 up

# Verify at0 is up
sleep 1
if ! ip link show at0 | grep -q "state UP"; then
  log_error "Failed to bring at0 interface up"
  exit 5
fi
...
if [[ -n "${UPLINK_IF:-}" && "$UPLINK_IF" != "none" ]]; then
  ...
  iptables -C FORWARD -i at0 -o "$UPLINK_IF" -j ACCEPT 2>/dev/null || \
    iptables -A FORWARD -i at0 -o "$UPLINK_IF" -j ACCEPT

  iptables -C FORWARD -i "$UPLINK_IF" -o at0 -m state --state RELATED,ESTABLISHED -j ACCEPT 2>/dev/null || \
    iptables -A FORWARD -i "$UPLINK_IF" -o at0 -m state --state RELATED,ESTABLISHED -j ACCEPT
...
ip addr show at0 | grep -E "inet |state"
echo
...
while true; do
  ...
  if [[ -f "$AIRBASE_PIDFILE" ]]; then
    AP_PID=$(cat "$AIRBASE_PIDFILE" 2>/dev/null || echo "")
    if [[ -n "$AP_PID" ]] && ! kill -0 "$AP_PID" 2>/dev/null; then
      log_error "airbase-ng (PID $AP_PID) stopped unexpectedly"
